...
async function getWatchlistWithNotes(userId) {
  const [{ data: wl }, { data: notes }, { data: portfolio }] = await Promise.all([
    supabase.from('watchlist').select('*').eq('user_id', userId),
    supabase.from('notes').select('*').eq('user_id', userId),
    supabase.from('portfolio').select('*').eq('user_id', userId),
  ])
  return { watchlist: wl || [], notes: notes || [], portfolio: portfolio || [] }
}
...
// ─── Build context ────────────────────────────────────────────────────────────
function buildContext({ profile, watchlist, notes, portfolio }) {
...
- Context: ${profile.additional_context || 'none'}` : 'No investor profile set yet.'
  ...
  return { profileSummary, watchlistTickers }
}
...
export default function AIAdviser({ initialTab = 'briefing' }) {
  ...
  const init = async () => {
    ...
    setProfile(prof)
    setContext(buildContext({ profile: prof, watchlist, notes, portfolio }))
    setPastInsights(insights)
    setLoading(false)
  }

  const systemPrompt = (ctx) => `You are Apex, a personal investment adviser for Sri — a data engineer in Fullerton, California who works with Azure Databricks, Python, Spark, and SQL professionally. Sri analyzes non-tech S&P 500 companies for AI adoption potential.

${ctx?.profileSummary || ''}

Sri's watchlist (${ctx?.watchlistTickers?.length || 0} companies):
${JSON.stringify(ctx?.watchlistTickers || [], null, 2)}

Metric guide: ROIC-WACC (>4% = value creating), FCF Yield (>4% = healthy), EV/EBITDA (lower = cheaper), Operating Leverage (positive = AI in margins), Rev/Employee Growth (positive = AI productivity), Debt/EBITDA (<2 = safe), PEG (<1.5 = fair value). Score 1-5 composite.

Rules: Be specific to Sri's actual stocks and data. Be honest about risks. Keep it concise and actionable. Frame as "worth investigating" not "buy/sell". End briefings with disclaimer. Reference his notes when relevant.`
              ...
              <div style={{display:'flex', flexDirection:'column', gap:12}}>
                {signals.map(stock => (
                  <div key={stock.ticker} style={{background:'var(--surface)', border:'1px solid var(--border)',
                    ...
                    <button onClick={()=>{
                      setActiveTab('chat')
                      setInput(`Stress test my investment thesis on ${stock.ticker}. Here are the signals I'm seeing: ${stock.signals.map(s=>s.label).join(', ')}. What are the risks I should think about?`)
        ...
        {activeTab==='sectors' && (
          <div style={{flex:1, overflowY:'auto', padding:'24px 28px'}}>
            ...
            <div style={{marginTop:16}}>
              <button onClick={()=>{
                setActiveTab('chat')
                setInput(`Analyze my sector distribution and tell me if I have any concentration risks or blind spots given my ${profile?.answers?.style || 'investment'} style and ${profile?.answers?.risk || 'moderate'} risk tolerance.`)
              }} style={{padding:'9px 18px', background:'var(--accent-dim)', border:'1px solid var(--accent-border)',
                color:'var(--accent)', borderRadius:'var(--radius)', fontSize:12, fontWeight:600, cursor:'pointer'}}>
              ...
              {messages.length === 0 ? (
                <div>
                  <div style={{fontSize:13, color:'var(--text2)', marginBottom:16, lineHeight:1.7}}>
                    Ask anything about your watchlist or investment strategy. Try asking Claude to stress-test a thesis,
                    compare two stocks, or explain a metric.
                  </div>
                  <div style={{display:'flex', flexDirection:'column', gap:8}}>
                    {SUGGESTED.map(s => (
                      <button key={s} onClick={()=>setInput(s)}
                        style={{padding:'10px 14px', background:'var(--surface)', border:'1px solid var(--border)',
                          borderRadius:'var(--radius)', color:'var(--text2)', fontSize:12, textAlign:'left',
                  ...
                  {messages.map((msg, i) => (
                    <div key={i} style={{display:'flex', justifyContent:msg.role==='user'?'flex-end':'flex-start'}}>
                      <div style={{maxWidth:'82%', padding:'12px 16px', borderRadius:'var(--radius-lg)',
                        ...
                        fontSize:13, lineHeight:1.8}}><Markdown text={msg.content} />
                      </div>
                    </div>
                  ))}
                  {chatLoading && (
                    <div style={{display:'flex', justifyContent:'flex-start'}}>
                      <div style={{padding:'12px 16px', background:'var(--surface)', border:'1px solid var(--border)',
                        borderRadius:'var(--radius-lg)', fontSize:13, color:'var(--muted)', fontFamily:'var(--font-mono)'}}>
                        ✦ thinking…
                      </div>
                    </div>
                  )}
            ...
            <div style={{padding:'14px 24px', borderTop:'1px solid var(--border)', background:'var(--surface)', flexShrink:0}}>
              <div style={{display:'flex', gap:10}}>
                <input value={input} onChange={e=>setInput(e.target.value)}
                  ...
                  placeholder="Ask about your watchlist, thesis, portfolio, or strategy…"
                  style={{flex:1, background:'var(--surface2)', border:'1px solid var(--border2)',
                    color:'var(--text)', padding:'10px 14px', borderRadius:'var(--radius)',
                ...
                <button onClick={sendMessage} disabled={!input.trim()||chatLoading}
                  style={{padding:'10px 18px', background:'var(--accent)', color:'#fff',
                    border:'none', borderRadius:'var(--radius)', fontSize:13, fontWeight:600,
                    opacity:!input.trim()||chatLoading?0.5:1, cursor:'pointer'}}>
                  Send
                </button>
              </div>
              <div style={{fontSize:10, color:'var(--muted)', marginTop:6, fontFamily:'var(--font-mono)'}}>
                Thesis builder: try "Stress test my thesis on [TICKER]" · Enter to send
              </div>
            </div>
          </div>
        )}
